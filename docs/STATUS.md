...
## This pass (public NOAA ingest, 2026-08-20)

Worker `buildTripPack({ tryLive })` and preview `GET /api/packs?live=1` now fetch public NOAA bytes when the network allows. Preview without `live=1` stays deterministic fixtures. Flutter was not started. Worker scoring was not added. AIS stays the demo gateway. Humor stayed out of helm toasts.
...
- GFS-Wave NOMADS `filter_gfswave.pl` Atlantic `atlocn.0p16` **f000** subset for the Point Judith box (~3 KB GRIB2, confirmed fetchable). SHA-256 is recorded on the manifest sources. **Not packed as the 72 h wind/wave grid** — helm paint stays on the fixture until cron can pace ~25 step-3 files.

### Still fixture / not done

- SST / chlorophyll / altimetry / bathymetry / contours / canyons / HMS grids and vectors.
- 72 h wind and wave **grids** (GFS-Wave hour 0 is hashed proof only; NDFD not fetched).
